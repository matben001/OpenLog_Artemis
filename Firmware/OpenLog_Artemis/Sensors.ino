  ...
  while (temp != NULL)
  {
    ...
    if (temp->online == true)
    {
      ...
      case DEVICE_BNO08x:
      {
        ...
        if (nodeDevice->wasReset())
        {
          Serial.print("sensor was reset ");
          if (!nodeDevice->enableReport(SH2_ROTATION_VECTOR, (uint16_t)(settings.usBetweenReadings)))
          {
            SerialPrintf1("Could not enable rotation vector in play");
          }
          else
          {
            SerialPrintf1("Rotation Vector configure in play")
          }
          // linear acceleration
          if (!nodeDevice->enableReport(SH2_LINEAR_ACCELERATION, (uint16_t)(settings.usBetweenReadings)))
          {
            SerialPrintf1("Could not enable linear acceleration in play");
          }
          else
          {
            SerialPrintf1("Linear Acceleration configure in play")
          }
        }
        if (nodeSetting->log == true)
        {
              ...
              (nodeSetting->logMag) || (nodeSetting->logEuler))
          {
            if (nodeDevice->getSensorEvent(&sensorValue)) // Check if new data is available (dangerous? what happens if data is not available?)
            {
              ...
              {
                olaftoa(sensorValue.un.rotationVector.real, tempData1, 2, sizeof(tempData1) / sizeof(char));
                olaftoa(sensorValue.un.rotationVector.i, tempData2, 2, sizeof(tempData2) / sizeof(char));
                olaftoa(sensorValue.un.rotationVector.j, tempData3, 2, sizeof(tempData3) / sizeof(char));
                sprintf(tempData, "%s,%s,%s,", tempData1, tempData2, tempData3);
                strcat(sdOutputData, tempData);
                olaftoa(sensorValue.un.rotationVector.k, tempData1, 2, sizeof(tempData1) / sizeof(char));
                olaftoa(sensorValue.un.rotationVector.accuracy, tempData2, 2, sizeof(tempData2) / sizeof(char));
                sprintf(tempData, "%s,%s,", tempData1, tempData2);
                strcat(sdOutputData, tempData);
              }
              if (nodeSetting->logLinAccel)
              {
                olaftoa(sensorValue.un.linearAcceleration.x, tempData1, 2, sizeof(tempData1) / sizeof(char));
                olaftoa(sensorValue.un.linearAcceleration.y, tempData2, 2, sizeof(tempData2) / sizeof(char));
                olaftoa(sensorValue.un.linearAcceleration.z, tempData3, 2, sizeof(tempData3) / sizeof(char));
                sprintf(tempData, "%s,%s,%s,", tempData1, tempData2, tempData3);
                strcat(sdOutputData, tempData);
              }
            }
    ...
    if (temp->online == true)
    {
      ...
      case DEVICE_BNO08x:
      {
        ...
        if (nodeSetting->log)
        {
          if (nodeSetting->logQuat)
          {
            strlcat(helperText, "QuatReal,", lenText);
            strlcat(helperText, "QuatI,", lenText);
            strlcat(helperText, "QuatJ,", lenText);
            strlcat(helperText, "QuatK,", lenText);
            strlcat(helperText, "QuatAcc,", lenText);
          }
          // linear accel
          if (nodeSetting->logLinAccel)
          {
            strlcat(helperText, "LinAccX,", lenText);
            strlcat(helperText, "LinAccY,", lenText);
            strlcat(helperText, "LinAccZ,", lenText);
          }
        }
      }
      break;
