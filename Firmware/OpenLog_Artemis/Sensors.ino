            ...
            if (nodeSetting->log == true)
            {
                ...
                (nodeSetting->logMag) || (nodeSetting->logEuler))
              {
                if (nodeDevice->dataAvailable() ) // Check if new data is available (dangerous? what happens if data is not available?)
                {
                  ...
                  {
                    float roll = (nodeDevice->getRoll()) * 180.0 / PI; // Convert roll to degrees
                    float pitch = (nodeDevice->getPitch()) * 180.0 / PI; // Convert pitch to degrees
                    float yaw = (nodeDevice->getYaw()) * 180.0 / PI; // Convert yaw / heading to degrees

                    sprintf(tempData, "%.02f,%.02f,%.02f", roll,pitch,yaw);
